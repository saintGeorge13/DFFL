...
class Agent:
    def __init__(self, *args):
        ...
        for k,v in self.model_list[0].state_dict().items():
            if 'filter_bank' in k:
                filter_banks[k] = copy.deepcopy(v)
        self.filter = filter_banks
        self.select = 1
        self.fair = 0
    ...
    def test_all(self, loader_test, timer_test, run, epoch):
        ...
        for i, model in enumerate(self.model_list):
            ...
            self.ckp.write_log('model {} accuracy: {:.3f}'.format(i, accuracy))
    ...
    def KD(self, input_p, input_q):
        kl_loss = nn.KLDivLoss(reduction="batchmean")
        p = F.softmax(input_p, dim = 1)
        q = F.log_softmax(input_q, dim = 1)
        result = kl_loss(q,p)
        return result   
    def train_local2(self, loader_train, budget, epochs):
        ...
        for epoch in range(epochs):
            ...
            for batch, (img, label) in enumerate(loader_train):                
                ...
                self.optimizer_list[model_id].zero_grad()
                out_of_local = model(img, return_feature=True)            
                ...
                for it in range(num_branch):
                    ...
                    this_ce = self.loss_func(this_log_prob, label)
                    this_kl = self.KD(this_log_prob, log_probs)
                    ce_branch += this_ce * (it + 1)
                    kl_branch += this_kl * (it + 1)

                
                ce_loss = self.loss_list[model_id](log_probs, label)[0] * (num_branch + 1)
                loss = (ce_loss + ce_branch + kl_branch) / (num_branch * (num_branch + 1))
        ...
        log_train = self.loss_list[model_id].log_train[-1,:]/n_samples
        self.model_list[model_id] = self.model_list[model_id].to('cpu')
        return sum(loss_list)/len(loss_list), log_train
    
    def train_local3(self, loader_train, budget, epochs):
        ...
        for epoch in range(epochs):
            ...
            for batch, (img, label) in enumerate(loader_train):                
                ...
                out_of_local = model(img, return_feature=True)            
                local_features = out_of_local[:-1]
                log_probs = out_of_local[model_id-4]
                if model_id != 3:
                    log_probs = global_model(log_probs, level=model_id + 3)
                
                ce_branch = 0
                kl_branch = 0
                num_branch = model_id + 2
                for it in range(num_branch):
                    this_log_prob = global_model(local_features[it], level=it + 1)
    ...
    def summarize(self, ckp):
        for i, _ in enumerate(self.model_list):
            with open(os.path.join(os.path.join(self.args.dir_save, self.args.save), 'config.txt'), 'a') as f:
                f.write('# parameters of model {}: {:,}\n'.format(i,
