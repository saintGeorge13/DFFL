...
from data.partition_data import partition_data, Partition
...
#     y_train = load_cifar10_data(args.dir_data)[1]
#     num_classes = 10
...
def get_agent_loader(args, kwargs):
    ...
    if not args.test_only:
        ...
        train_dataset = datasets.CIFAR10(args.dir_data, train=True, transform=transform_train, download=True)
        agent_dataid = partition_data(train_dataset, args, args.n_agents)

    for i in range(args.n_agents):
        # train_ds = CIFAR10_truncated(root=args.dir_data, dataidxs=agent_dataid[i], train=True, transform=transform_train, download=True)
        ...
        train_loader = DataLoader(Partition(train_dataset, list(agent_dataid[i])), batch_size=args.batch_size, shuffle=True)
        loaders_train.append(train_loader)
    offset_worker, offset_sequence, order = cluster(args, train_dataset, agent_dataid, loaders_train)
    return loaders_train, offset_worker, offset_sequence, order   
